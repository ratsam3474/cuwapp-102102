...
async def lifespan(app: FastAPI):
    ...
    init_database()
    # Import admin_users model to register it with Base
...
@app.post("/auth/login")
async def login(request: LoginRequest, response: Response, db: Session = Depends(get_db_dependency)):
    """Login admin user"""
...
async def logout(request: Request, response: Response, db: Session = Depends(get_db_dependency)):
    """Logout admin user"""
    return await logout_admin(request=request, response=response, db=db)

@app.get("/", response_class=HTMLResponse)
async def admin_dashboard(request: Request, db: Session = Depends(get_db_dependency)):
...
async def get_overview_stats(
    ...
    try:
        ...
        # Get Docker container stats - specifically look for WAHA containers
        container_count = 0
        waha_containers = 0
        if docker_client:
            ...
            for c in containers:
                ...
                if 'waha' in name_lower or 'whatsapp' in name_lower:
                    waha_containers += 1
                    if c.status == 'running':
                        container_count += 1
            logger.info(f"Found {waha_containers} WAHA containers, {container_count} running")
...
async def get_whatsapp_sessions(db: Session = Depends(get_db_dependency)):
    """Get all WhatsApp sessions from the database"""
    try:
        # Query the user_whatsapp_sessions table
        result = db.execute(text("""
            SELECT 
                uws.*,
                us.email as user_email,
                us.plan_type
            FROM user_whatsapp_sessions uws
            LEFT JOIN user_subscriptions us ON uws.user_id = us.user_id
        ...
        for row in result:
            sessions.append({
                ...
                "campaigns_run": row.campaigns_run,
                "plan_type": row.plan_type
            })
        
        return sessions
    except Exception as e:
        logger.error(f"Error getting WhatsApp sessions: {e}")
        return []
...
async def get_docker_containers():
    ...
    try:
        ...
        for container in containers:
            # Look for WAHA containers specifically
            name_lower = container.name.lower()
            if 'waha' in name_lower or 'whatsapp' in name_lower or 'cuwhapp' in name_lower:
                # Get container stats
                try:
...
async def email_templates_page(request: Request, db: Session = Depends(get_db_dependency)):
    """Email templates management page - protected"""
    admin_user = await get_current_admin(request, db)
    if not admin_user:
        return RedirectResponse(url="/login", status_code=302)
...
async def get_waha_instances():
    ...
    # Try to get WAHA status even without pool manager
    instances = []
    
    # Check Docker containers for WAHA
    if docker_client:
        try:
            containers = docker_client.containers.list(all=True)
            for container in containers:
                if 'waha' in container.name.lower():
                    instances.append({
                        "name": container.name,
                        "status": container.status,
    ...
    # If pool manager is available, get additional info
    if not waha_monitoring_enabled:
        return {"instances": instances, "pool_status": "Pool manager not available"}
...
async def waha_monitoring_page(request: Request, db: Session = Depends(get_db_dependency)):
    """WAHA monitoring dashboard page - protected"""
    admin_user = await get_current_admin(request, db)
    if not admin_user:
        return RedirectResponse(url="/login", status_code=302)
