...
class SessionManager:
    ...
    def upgrade_session_plan(self, user_id: str, new_plan_type: PlanType) -> Dict[str, Any]:
        """Upgrade an existing session to a new plan type (typically after payment)"""
        try:
            ...
            if not user_session_id:
                logger.warning(f"No active session found for user {user_id} to upgrade")
                return {
                    "success": False,
                    "error": "No active session found for user"
                }
            
            session = self.active_sessions[user_session_id]
            old_plan = session["plan_type"]
            
            # Update session with new plan configuration
            config = SESSION_CONFIG.get(new_plan_type, SESSION_CONFIG[PlanType.FREE])
            
            # Update session properties
            session["plan_type"] = new_plan_type
            session["persistent"] = config["persistent"]
            session["refresh_allowed"] = config["refresh_allowed"]
            
            # Recalculate expiration for upgraded plan
            now = datetime.utcnow()
            if config["timeout_minutes"]:
                # Should not happen for upgrades, but handle it
                session["expires_at"] = now + timedelta(minutes=config["timeout_minutes"])
            else:
                # Persistent session - extend to 30 days from now
                session["expires_at"] = now + timedelta(days=30)
            
            logger.info(f"Session upgraded for user {user_id} from {old_plan.value} to {new_plan_type.value}")
            ...
            return {
                ...
                "message": f"Session upgraded from {old_plan.value} to {new_plan_type.value} plan"
            }
            
        except Exception as e:
            logger.error(f"Failed to upgrade session: {str(e)}")
            return {
                "success": False,
                "error": str(e)
            }
