        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("body", {}).get("success"):
                    ...
                    cursor = conn.cursor()
                    cursor.execute("""
                        INSERT INTO waha_instances (instance_id, instance_url, current_sessions, status)
                        VALUES (?, ?, 0, 'active')
                    """, (port, endpoint))
                    conn.commit()
                    conn.close()
                    
                    logger.info(f"Created WAHA instance via DO Function on port {port}")
                    return endpoint
                else:
                    error = result.get("body", {}).get("error", "Unknown error")
                    logger.error(f"DO Function failed: {error}")
            else:
                logger.error(f"DO Function returned status {response.status_code}")
                
        except Exception as e:
            logger.error(f"Error calling DO Function: {e}")
        
        # Fallback to free instance
        return self.free_instance_url
    
    def create_new_instance(self) -> str:
        """Create a new WAHA Docker container"""
        # Check if DO Function is configured
        do_function_url = os.getenv("DO_WAHA_FUNCTION_URL")
        if do_function_url:
            return self.create_instance_via_do_function()
